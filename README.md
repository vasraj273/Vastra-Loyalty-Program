...
- `VASTRA_API_BASE_URL` / `VASTRA_API_KEY` (env) — Vastra's API origin + `api-key`, called server-side only (`app/vastra_client.py`) to power the panel's **Vastra OTP login** (`/auth/vastra/send-otp` → `/auth/vastra/verify-otp`, which stores the per-org Vastra `access_token`) and the Products tab / QR generation (`GET /vastra/products` uses that stored token; password-only sessions get `409`). Optional `VASTRA_API_TIMEOUT` (default `10`s), `VASTRA_UDID` / `VASTRA_DEVICE_TYPE` (required Vastra device headers, fixed defaults). Unset → OTP login and `GET /vastra/products` fail closed with `502`.
- `YOURAPP_API_KEY` (env) — shared secret enabling the **YourApp server-to-server scan** endpoints (`POST /yourapp/qr/lookup`, `POST /yourapp/scan`; header `X-API-Key`). Unset → those endpoints return `503`. Server-side only, never in a mobile build.
