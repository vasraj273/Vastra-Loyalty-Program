...
Password login for retailers. Default initial password is `<username>123` (derived from lowercased first word of shop name). Returns `must_change: true` while the account is still on that default password — advisory only, so the client can show a "change your password" reminder. It does not gate access.
...
- **Request:** `{ "csv": "shop_name,name,region,phone,distributor,points,external_id\n…" }` (a shop *or* name column is required; everything else is optional). The shop-or-name rule applies **per row**: when a file has both, a row with a blank shop cell is filed under the person's name instead of being rejected — only a row with neither errors as `missing shop name`. Headers are matched against alias lists, not dictated: `Mobile`→phone, `City`→region, `address1`→address, `Distributor`/`Dealer`/`Agency`/`Parent`→distributor, `Point Balance`/`Balance`→points. `external_id` is read only from a column literally named that, and is unique per manufacturer.
